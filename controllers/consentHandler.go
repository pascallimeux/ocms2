...
func (a *AppContext) processConsent(w http.ResponseWriter, r *http.Request) {
	...
	err1 := a.AuthContext.CheckPermissionFromToken(w, r, "processConsent", "")
	if err1 != nil {
		return
	}
...
func (a *AppContext) processConsentTR(w http.ResponseWriter, r *http.Request) {
	...
	if transaction.Txid == "" {
		common.SendError(log.Here(), w, errors.New("This is not a consent creation transaction"))
		return
	}
	if err != nil {
		common.SendError(log.Here(), w, err)
...
func (a *AppContext) createConsent(consent model.Consent) ([]byte, error) {
...
func (a *AppContext) isConsent(consent model.Consent) ([]byte, error) {
...
func convertHPConsents2APIConsents(HPconsents []hyperledger.Consent) []model.Consent {
	log.Trace(log.Here(), "convertHPConsents2APIConsents() : calling method -")
	consents := make([]model.Consent, len(HPconsents))
...
func convertHPConsent2APIConsent(HPconsent hyperledger.Consent) model.Consent {
...
func consent2Bytes(consent model.Consent) ([]byte, error) {
...
func check_args(consent *model.Consent) error {
